...
def adjust_bright_and_contrast(input_img, brightness = 0, contrast = 0):
    ...
    if brightness != 0:
        ...
        buf_img = cv2.addWeighted(buf_img, alpha_bright, buf_img, 0, gamma_bright)
    ...
    if contrast != 0:
        ...
        buf_img = cv2.addWeighted(buf_img, alpha_contrast, buf_img, 0, gamma_contrast)
...
def sepia(img):
    ...
    sepia_kernel = np.matrix([
        ...
        [0.349, 0.686, 0.168],
        [0.393, 0.769, 0.189]])
    img_sepia = cv2.transform(img_sepia, sepia_kernel) # multipying image with special sepia matrix
...
original_image = cv2.cvtColor(uploaded, cv2.COLOR_BGR2RGB)
